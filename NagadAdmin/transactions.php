...
<?php
  ...
  //$sql="SELECT id,name,mobile,email,created_at FROM customers";
  $sql="SELECT users.business_name,users.email,users.mobile AS vendor_mobile,customers.name,customers.mobile,transactions.id,transactions.payment_type,transactions.amount,transactions.description,transactions.udhaar_date,transactions.udhaar_clear,transactions.created_at FROM transactions LEFT JOIN customers ON(customers.id=transactions.user_id) LEFT JOIN users ON(users.id=transactions.vendor_user_id)";
  $exe = $db->query($sql);
...
foreach ($data as $key => $item){
        ...
        if($item['payment_type']==1){
          ...
          if($item['udhaar_clear']==1){
            echo'<td>Udhaar Cleared</td>';
          }else{
            echo'<td>Udhaar due on '.$item['udhaar_date'].'</td>';
          }
          ...
          echo'<td>Gift Card</td>';
        }else if($item['payment_type']==7){
          echo'<td>Any Other</td>';
        }else{
          echo'<td>Others</td>';
        }
