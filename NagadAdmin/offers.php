...
<?php
  ...
  $sql="SELECT users.business_name,users.email,users.mobile AS vendor_mobile,offers.id,offers.offer_name,offers.offer_details,offers.start_date,offers.end_date,offers.image,offers.created_at FROM offers INNER JOIN users ON(users.id=offers.user_id)";
  $exe = $db->query($sql);
...
foreach ($data as $key => $item){
        ...
        echo'<td>'.$item['offer_details'].'</td>';
        echo'<td>'.$item['start_date'].'</td>';
        echo'<td>'.$item['end_date'].'</td>';
        echo'<td><img src="'.$item['image'].'" alt="Offer Image" height="50" width="50"></td>';
